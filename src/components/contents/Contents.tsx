...
export const Contents = ({ children, className }: ContentsProps) => {
    ...
    return (
        <section
            ...
            role="main"
            tabIndex={-1}
            className={`absolute top-10 right-0 flex flex-col sm:pb-10 transition-width transition-transform ease-in-out overflow-x-hidden overflow-y-auto gap-4 scrolls min-h-[calc(100dvh-40px)]
                max-h-[calc(100dvh-40px)]
                py-2 sm:py-4 md:py-6 ${isMobileSize ? "px-2 pb-16" : "px-3 sm:px-4 md:px-6 pb-2"} ${isFullscreen ? "justify-center" : ""} ${className} ${backgroundClass} ${textColor}`}
            style={{
